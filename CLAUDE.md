...
- Pivot/swing (`_pivot_body`): for each gripping hand, target = the point at orbit radius from the anchor in the *direction* of the cursor; the torso's `linear_velocity` is driven toward the average target (`swing_strength`, capped by `max_swing_speed`). The body orbits the grab point; the cursor sets the angle. On a **bar** grab only, while the pose key is held the orbit radius eases out toward full arm reach (`straighten_speed`), so the arm pulls taut into a straight rigid spoke and the swing stays smooth (capped per-grab at the arm's slack so the hand can't detach); key up freezes the radius. Non-bar grabs keep a fixed radius — straightening into a flat surface jams the arm against the reach-clamp and makes the body spasm. Each frame it also tracks the body's angular speed `ω` around the anchor (in the play plane, EMA-smoothed by `spin_smooth`).
